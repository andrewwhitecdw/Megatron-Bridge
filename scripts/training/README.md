...
Use `--segment N` when the target Slurm cluster requires a topology-aware
segment size. The value is emitted as an sbatch allocation parameter; it is
not forwarded to the training command or its srun step.
...
The compact launcher does not add rank-command prefixes or infer a segment.
Select topology and NUMA policies explicitly for the target cluster.
